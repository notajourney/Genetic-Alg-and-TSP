...
def GA_TSP_V2(n, max_evals, selectfct, fitnessfct,fitnessfctSA, crossoverfct, mutationfct,graph,  seed=None):
    eval_cntr = 0
    history = []
    mu = 20
    pc = 0.37
    ...
    """Individuals go to self improvement using SA"""
    for i in range(mu):
        individual =  np.copy(population[i])
        population[i]= sa.SimulatedAnnealing (n,sa_evals,fitnessfctSA,individual,graph,seed)
        eval_cntr+=sa_evals
...
if __name__ == "__main__":
    ...
    for i in range(Nruns):
        xmin,fmin,history = GA_TSP_V2(n, evals, select_proportional_v2,
                                    compute_tour_length_v2(graph),compute_tour_for_perm ,crossover_v2, mutate_perm_v2,graph)
        #GA_TSP_V2(n, max_evals, selectfct, fitnessfct,fitnessfctSA, crossoverfct, mutationfct,graph,  seed=None):
